...
export default function PropertiesPage({ setPage, setSelected, isFav, onFav }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (rooms)
      r = r.filter((p) =>
        rooms === "4+" ? p.rooms >= 4 : p.rooms === parseInt(rooms),
      );
          ...
          <div className="filter-section">
            ...
            <div className="filter-chips">
              {["venta", "alquiler"].map((o) => (
                <span
                  key={o}
                  className={`filter-chip ${op === o ? "active" : ""}`}
                  onClick={() => {
                    setOp(op === o ? "" : o);
                    setCurrentPage(1);
                  }}
          ...
          <div className="filter-section">
            ...
            <div className="filter-chips">
              {["1", "2", "3", "4+"].map((r) => (
                <span
                  key={r}
                  className={`filter-chip ${rooms === r ? "active" : ""}`}
                  onClick={() => {
                    setRooms(rooms === r ? "" : r);
                    setCurrentPage(1);
                  }}
